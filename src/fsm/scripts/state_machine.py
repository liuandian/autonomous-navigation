...
class StateMachine:
    ...
    def handle_init(self):
        rospy.loginfo_throttle(5, "[init] 执行初始化逻辑中...")


    def handle_boxes_count(self):
        rospy.loginfo_throttle(5, "[boxes_count] 执行数字识别逻辑中...")

    def handle_cross_bridge(self):
        rospy.loginfo_throttle(5, "[cross_bridge] 执行跨桥任务中...")

    def handle_find_goal(self):
        
        rospy.loginfo_throttle(5, "[find_goal] 寻找目标点中...")
...
if __name__ == "__main__":
    ...
    # sm.run()
    # rospy.spin()
    get_goal_pose_from_config_map = sm.get_goal_pose_from_config_map("goal")